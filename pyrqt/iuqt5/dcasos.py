...
from pyrqt.ui.dialogos.uidcasos import DialogoCasos
...
class DCasos(DialogoCasos):
    ...
    def mostrar(self, variable):
        ...
        if self.__dato.getVar(self.__variable).tipo != "Entero":
            rc = QMessageBox.warning(
                self, "Error", "En esta versión solo los Enteros aceptan casos"
            )
            self.reject()
        else:
            self.show()
    ...
    def myUpdate(self):
        self.textLabel1.setText(
            "Variable:" + self.__dato.getVar(self.__variable).nombre
        )
        self.listBox1.clear()
    ...
    def __addclave(self):
        ...
        if (
            self.__variable != None
            and self.lineEdit2.text().latin1()
            ...
            self.__dato.getVar(self.__variable).valores[
                self.lineEdit1.text().latin1()
            ] = self.lineEdit2.text().latin1()
            self.show(self.__variable)
        else:
            rc = QMessageBox.warning(
                self, "Atencion", "Falta el valor o clave", "Volver al dialogo", "", 0
            )

    def __borrarclave(self):
        """Borra la clave señalada por listBox1"""
        del self.__dato.getVar(self.__variable).valores[
            self.listBox1.currentText().latin1().split(" ")[0]
        ]
        self.show(self.__variable)
