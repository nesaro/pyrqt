...
from pyrqt.iuqt5.ui.dcrevar import Ui_DialogoCrevar
...
class DCrevar(QDialog):
        ...
        if self.ui.lineEdit1.text():
            ...
            if sobreescritura:
                if self.ui.checkBox2.isChecked():
                    solofiltrado = True
            try:
                self.__idu.ana_var_expresion(
                    self.ui.lineEdit1.text(),
                    self.ui.comboBox1.currentText(),
                    "NA",
                    self.ui.lineEdit2.text(),
                    self.ui.textEdit1.toPlainText(),
                    permitirsobreescritura=sobreescritura,
                    solofiltrados=solofiltrado,
                )
                ...
                QErrorMessage(self, "error").message("La expresión no es correcta")
                self.__idu.borrar_var(
                    self.ui.lineEdit1.text().latin1()
                )  # Borrar la variable que está a medias
            except (ZeroDivisionError, OverflowError):
                QErrorMessage(self, "error").message(
                    "La expresión genera un desbordamiento"
                )
            except VariableExisteException:
                QMessageBox.warning(self, "atención", "La variable ya existe")
            except TypeError:
                QErrorMessage(self, "error").message(
                    "El tipo de variable no coincide con el existente"
                )
                LOG.exception("Excepción al añadir la variable con la expresión")
            else:
            ...
            returncode = QtGui.QMessageBox.warning(
                self,
                "Atencion",
    ...
    def __conexiones(self):
        ...
        self.connect(
            self.ui.pushButton3,
            SIGNAL("clicked()"),
            self.__insertar_funcion_seleccionada,
        )
        self.connect(
            self.ui.pushButton4, SIGNAL("clicked()"), self.__insertar_variable_seleccion
        )
        self.connect(
            self.ui.comboBox1, SIGNAL("activated(int)"), self.__actualizar_cajas
        )
        self.connect(
            self.ui.listWidget_2,
            SIGNAL("itemSelectionChanged()"),
            self.__actualizar_lista_funciones,
        )
        self.connect(
            self.ui.pushButton5, SIGNAL("clicked()"), self.__insertar_simbolo_mas
        )
        self.connect(
            self.ui.pushButton6, SIGNAL("clicked()"), self.__insertar_simbolo_menos
        )
        self.connect(
            self.ui.pushButton7, SIGNAL("clicked()"), self.__insertar_simbolo_asterisco
        )
        self.connect(
            self.ui.pushButton8, SIGNAL("clicked()"), self.__insertar_simbolo_barra
        )
        self.connect(
            self.ui.pushButton9,
            SIGNAL("clicked()"),
            self.__insertar_simbolo_2_asteriscos,
        )
    ...
    def __actualizar_lista_funciones(self):
        ...
        if cmp(self.ui.listWidget_2.currentItem().text(), "Aritméticas") == 0:
            # Estamos con las aritmeticas
            self.ui.listWidget_3.insertItem("+")
            self.listBox3.insertItem("-")
            self.listBox3.insertItem("*")
            self.listBox3.insertItem("/")
        elif cmp(self.ui.listWidget_2.currentItem().text(), "Lógicas") == 0:
            self.listBox3.insertItem("and")
            self.listBox3.insertItem("or")
        else:
            lista = self.__gestorpaquetes[
                str(self.ui.listWidget_2.currentItem().text())
            ].values()
