...
class InterfazQt5(QApplication):
    def __init__(
        self,
        interfazconfig,
        portero,
        ...
        self.__dsplash = DSplash(
            self.vprincipal, gestortemas, self.__iconfig, self.vsalida
        )
        self.doperaciones = DOperaciones(
            self.vprincipal, idu, interfazoperaciones, self.vsalida
        )
    ...
    def loop(self):
        ...
        if self.__options.vsplash == None:
            if (
                self.__iconfig.configuracion["vsplash"]
                and not self.__gestorproyectos.fichero
            ):
                self.__dsplash.show()
        ...
        if self.__nuevo:
            ...
            QMessageBox.information(
                self.vprincipal,
                "Bienvenido",
                "Parece que es la primera vez que ejecutas Driza",
            )
        if self.__ioperaciones.listamodulosdefectuosos:
            ...
            for modulo in self.__ioperaciones.listamodulosdefectuosos:
                ristra += modulo + "\n"
            mensaje = (
                "Las siguientes operaciones no han podido ser cargadas:\n" + ristra
            )
            QMessageBox.information(
                self.vprincipal,
                QtCore.QCoreApplication.translate(
                    "VPrincipal", "Error loading operations"
                ),
                mensaje,
            )
