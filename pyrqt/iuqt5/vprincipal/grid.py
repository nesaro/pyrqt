...
class Grid(QTabWidget):
    ...
    def __init__(self, padre, interfazdatos, porterodatos):
        ...
        self.modoetiqueta = (
            False  # Variable que guarda si estamos en modo etiqueta o normal
        )
    ...
    def borrar_seleccion(self, borrardatos=False):
        ...
        if borrardatos:
            ...
            for seleccion in range(tabla.numSelections()):
                for fila in range(
                    tabla.selection(seleccion).topRow(),
                    tabla.selection(seleccion).bottomRow() + 1,
                ):
                    lista.append(fila)
            lista.sort()
            lista.reverse()
        ...
        if tabla.numSelections():
            ...
            for fila in range(toprow, bottomrow + 1):
                ...
                for columna in range(leftcol, rightcol + 1):
                    if self.__idu[fila][columna].valido() and tabla.isSelected(
                        fila, columna
                    ):
    ...
    def __modificacion_t_var(self, fila, columna):
        ...
        if self.table2.currentRow() != fila or self.table2.currentColumn() != columna:
            return
        ...
        if columna == 1:  # El usuario quiere cambiar el tipo
            ...
            if metododeseado:
                pareja = self.__agenteconversion(
                    variable, textoencuestion, metododeseado
                )  # Pareja variable-list
            ...
            try:
                self.__idu.modificar_var(
                    variable, columna, str(self.table2.item(fila, y).text())
                )
            except VariableExisteException:
                QMessageBox.warning(self, "Atención", "El nombre de variable ya existe")
    ...
    def __actualizar_reg_interfazdatos(self, row, col, valor):
        ...
        try:
            ...
            QMessageBox.warning(self, "Atención", "El dato no es válido")
    ...
    def __modificacion_t_reg(self, fila, columna):
        ...
        if self.table1.currentRow() != fila or self.table1.currentColumn() != columna:
            return
        if (
            (fila < self.__idu.n_reg)
            and (columna < self.__idu.n_var)
            and (
                self.table1.item(fila, columna).text() == str(self.__idu[fila][columna])
            )
        ):
            return
        ...
        if columna >= self.__idu.n_var:
            LOG.debug(
                "Creando nueva variable por demanda en la modificaicon de un registro"
            )
        ...
        if fila >= self.__idu.n_reg:
            ...
            LOG.debug(
                "Creando nuevo registro por demanda en la modificaicon de un registro"
            )
    ...
    def __preguntar_conversion(self, variable, objetivo):
        ...
        if not isinstance(objetivo, str):
            LOG.debug(
                "__preguntar_conversion: Bad type for string: "
                + str(objetivo.__class__)
            )
            raise TypeError
        lista = []
        ...
        cadena = QInputDialog.getItem(
            self, "Elige!", "Elige una función de conversion", qlista, 0, False
        )
