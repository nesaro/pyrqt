...
class SelectorElementosEstudio(QWidget):
    ...
    def __init__(self, interfazdato, cajadisponible):
        ...
        WSelectorSimpleLayout = QHBoxLayout(self)  # ,11,6)
        layout2 = QVBoxLayout(None)  # ,0,6,"layout2")
        widgetStack1 = QStackedWidget(self)
        ...
        self.__pushButton1 = QPushButton(self)
        ...
        self._textLabel2 = QLabel(self)
        layout3.addWidget(self._textLabel2)

        self._cajaseleccion = QListWidget(self)
        ...
        self._textLabel2.setText("Selección")
...
class SelectorVariableVariable(SelectorElementosEstudio):
    ...
    def __init__(self, dato):
        ...
        SelectorElementosEstudio.__init__(
            self, dato, WidgetSeleccionCajaCaja("Variable1", "Variable2")
        )
    ...
    def _add_element(self):
        ...
        if (
            self._cajadisponible.caja1.currentText()
            and self._cajadisponible.caja2.currentText()
        ):
            self.seleccion.append(
                [
                    self._cajadisponible.caja1.currentText().latin1(),
    ...
    def _actualizar_caja_disponible(self):
        ...
        for var in self._dato.lista_tit():
            if not var in self.seleccion:
                lista.append(var)
        LOG.debug(
            "_actualizar_caja_disponible: caja1:"
            + str(self._cajadisponible.caja1.__class__)
        )
...
class SelectorDiscriminadorSimple(SelectorElementosEstudio):
    ...
    def __init__(self, dato):
        ...
        SelectorElementosEstudio.__init__(
            self, dato, WidgetSeleccionCajaCaja("Variable", "Caso")
        )
        self._textLabel2.setText("<b>Caso de la variable</b>")
        self.connect(
            self._cajadisponible.caja1,
...
class SelectorDiscriminadorDoble(SelectorElementosEstudio):
    ...
    def __init__(self, dato):
        ...
        SelectorElementosEstudio.__init__(
            self, dato, WidgetSeleccionComboCombo("Variable")
        )
        self.connect(
            self._cajadisponible.caja,
            SIGNAL("selectionChanged()"),
    ...
    def _add_element(self):
        if self._cajadisponible.caja.currentText():
            lista = [
                self._cajadisponible.caja.currentText,
    ...
    def __actualizar_combos(self):
        ...
        listacasos = [
            str(x)
            for x in self._dato.obtener_casos(
                self._cajadisponible.caja.currentText().latin1()
            )
        ]
...
class SelectorVariableDiscriminadorSimple(SelectorElementosEstudio):
    ...
    def __init__(self, dato):
        ...
        SelectorElementosEstudio.__init__(
            self,
            dato,
            WidgetSeleccionCajaCajaCajaComboCombo("Variable", "Caso", "Variable4"),
        )
        self._textLabel2.setText("<b>Caso de la variable</b>")
        self.connect(
            self._cajadisponible.zona1.caja1,
            SIGNAL("selectionChanged()"),
            self.__updatecaja2,
        )
        self.connect(
            self._cajadisponible.zona2.caja,
            SIGNAL("selectionChanged()"),
            self.__actualizar_combos,
        )
    ...
    def _add_element(self):
        """Añade un elemento de la seleccion de la cajadisponible"""
        if (
            self._cajadisponible.zona1.caja1.currentText()
            and self._cajadisponible.zona1.caja2.currentText()
            and self._cajadisponible.zona2.caja.currentText()
        ):
            array1 = [
                self._cajadisponible.zona1.caja1.currentText().latin1(),
                self._cajadisponible.zona1.caja2.currentText().latin1(),
            ]
            array2 = [
                self._cajadisponible.zona2.caja.currentText().latin1(),
                self._cajadisponible.zona2.combo1.currentText().latin1(),
                self._cajadisponible.zona2.combo2.currentText().latin1(),
            ]
...
class SelectorVariableDiscriminadorDoble(SelectorElementosEstudio):
    ...
    def __init__(self, dato):
        ...
        super().__init__(
            dato, WidgetSeleccionCajaComboComboCaja("Discriminador", "Variable")
        )
        # self.connect(self._cajadisponible.zona1.caja,SIGNAL("selectionChanged()"),self.__actualizar_combos) #todo new syntax

    def _add_element(self):
        """Añade un elemento de la seleccion de la cajadisponible"""
        if (
            self._cajadisponible.zona1.caja.currentText()
            and self._cajadisponible.zona2.caja.currentText()
        ):
            if (
                self._cajadisponible.zona1.combo1.currentText()
                != self._cajadisponible.zona1.combo2.currentText()
            ):
                array1 = [
                    self._cajadisponible.zona1.caja.currentText().latin1(),
                    self._cajadisponible.zona1.combo1.currentText().latin1(),
                    self._cajadisponible.zona1.combo2.currentText().latin1(),
                ]
...
class SelectorVariableVariableDiscriminadorDoble(SelectorElementosEstudio):
    ...
    def __init__(self, dato):
        ...
        SelectorElementosEstudio.__init__(
            self,
            dato,
            WidgetSeleccionCajaComboComboCajaCaja(
                "Discriminador", "Variable1", "Variable2"
            ),
        )
        self.connect(
            self._cajadisponible.zona1.caja,
            SIGNAL("selectionChanged()"),
            self.__actualizar_combos,
        )
    ...
    def _add_element(self):
        ...
        if (
            self._cajadisponible.zona1.caja.currentText()
            and self._cajadisponible.zona2.caja1.currentText()
            and self._cajadisponible.zona2.caja2.currentText()
        ):
            array1 = [
                self._cajadisponible.zona1.caja.currentText().latin1(),
                self._cajadisponible.zona1.combo1.currentText().latin1(),
            ...
            array2 = [
                self._cajadisponible.zona2.caja1.currentText().latin1(),
                self._cajadisponible.zona2.caja2.currentText().latin1(),
            ]
            self.seleccion.append([array1, array2])
            self._actualizar_cajas()
