...
from PyQt5.QtWidgets import QLabel, QTreeWidget, QTreeWidgetItem, QErrorMessage, QDialog
from pyrqt.iuqt5.ui.doperaciones import Ui_DialogoOperaciones
...
def listview_elementos_finales(listviewitem):
    ...
    if not listviewitem.child(0):
        ...
        while elemento:
            ...
            elemento = elemento.parent().child(
                elemento.parent().indexOfChild(elemento) + 1
            )
        return formarcategoriasdesdehijos(listviewitem, listahijos)
...
class DOperaciones(QDialog):
    ...
    def accept(self):
        ...
        try:
            resultado = self.__gestoroperaciones[nombre].procedimiento(
                widget.seleccion(), widget.opciones()
            )
        except OpcionesIncorrectaException:
            errormessage = QErrorMessage(self)
            errormessage.showMessage("Las opciones no son correctas")
        # except IndexError:
        #    errormessage = QErrorMessage(self)
    ...
    def __init_arbol(self):
        ...
        listalistasetiquetas = [
            operacion.etiquetas for operacion in self.__gestoroperaciones.values()
        ]
    ...
    def __render_widget(self, nombre, diccionariowidget):
        ...
        widget = WidgetOperacionSelectorOpcion(
            nombre,
            SL.TIPOSWIDGETOPERACIONESQT5[diccionariowidget["tipo"]]["seleccion"],
            diccionariowidget["opciones"],
            self.__idu,
        )
        return widget
        ...
        if nombre in self.__gestoroperaciones:
            if self.__gestoroperaciones.funcionchequeocondiciones(nombre):
                self.ui.stackedWidget.setCurrentWidget(
                    self.__widgets["operaciones"][nombre]
                )
            else:
                self.ui.stackedWidget.setCurrentWidget(
                    self.__widgets["otros"]["condicionesincorrectas"]
                )
        elif any(nombre in x for x in self.__gestorcategorias.categorias.values()):
            category_set = [
                x for x in self.__gestorcategorias.categorias.values() if nombre in x
            ][0]
            category_set = frozenset(category_set)
            self.ui.stackedWidget.setCurrentWidget(
                self.__widgets["categorias"][category_set]
            )
        else:
            self.ui.stackedWidget.setCurrentWidget(
                self.__widgets["otros"]["noimplementado"]
            )
