...
from PyQt5.QtWidgets import QToolButton, QWidget, QMainWindow
from pyrqt.iuqt5.ui.vsalida import Ui_VentanaSalida


class VSalida(QMainWindow):
    ...
    def __conexiones(self):
        ...
        self.ui.actionAbrir.triggered.connect(self.__dabrirfichero)
        # TODO convert to new syntax? if it works https://www.tutorialspoint.com/pyqt5/pyqt5_quick_guide.htm
        # self.connect(self.ui.actionAbrir,pyqtSignal("triggered()"),self.__dabrirfichero)
        # self.connect(self.ui.actionGuardar,pyqtSignal("triggered()"),self.__dguardarfichero)
        # self.connect(self.ui.actionExportar,pyqtSignal("triggered()"),self.__dexportar)
        # self.connect(self.ui.actionSalir,pyqtSignal("triggered()"),self.hide)
        # self.connect(self.__boton1,pyqtSignal("clicked()"),self.__dabrirfichero)
        # self.connect(self.__boton2,pyqtSignal("clicked()"),self.__dguardarfichero)
        # self.connect(self.__boton3,pyqtSignal("clicked()"),self.__dexportar)

        # self.connect(self.__boton4,pyqtSignal("clicked()"),self.__parent.vprincipal.show)
        # self.connect(self.ui.pushButton1,pyqtSignal("clicked()"),self.__borrar_elemento)
    ...
    def __dabrirfichero(self):
        ...
        nombrefichero = QtGui.QFileDialog.getOpenFileName(
            filter=filterlist, parent=self, caption="Dialogo abrir fichero"
        )
        ...
        if filename:
            try:
                ...
                QErrorMessage(self, "Error").message(
                    "Error en la carga de fichero, Probablemente versión anterior"
                )
            else:
                self.__myUpdate()
    ...
    def __dguardarfichero(self):
        ...
        if filename:
            ...
            try:
                ...
                fichero = e.fichero
                codigoretorno = QtGui.QMessageBox.information(
                    self,
                    "Atencion:",
                    "El fichero" + str(fichero) + " ya existe",
                    "Sobreescribir",
                    "Otro nombre",
                    "Cancelar",
                    0,
                ...
                QtGui.QMessageBox.warning(
                    self,
                    "Atención",
                    "La extensión del fichero es incorrecta.\nPruebe con otra extensión",
                )
